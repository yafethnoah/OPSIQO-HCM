...
Write-Host " OPSIQO H48.4 ATTENDANCE VISIBILITY + TIMESTAMP VALIDATION" -ForegroundColor Magenta
Write-Host "============================================================" -ForegroundColor Magenta
...
foreach ($Required in @(
    ...
    ".\src\lib\time\frontline-service.ts",
    ".\src\app\api\organizations\[orgId]\time\attendance-activity\route.ts",
    ".\scripts\opsiqo-h48-4-attendance-visibility-audit.mjs",
    ".\tests\h48-4-attendance-visibility.test.ts"
)) {
    if (-not (Test-Path -LiteralPath $Required -PathType Leaf)) {
        throw "STOP - required H48.4 file missing: $Required"
    }
}
...
Write-Host ""
Write-Host "=== H48.4 STATIC AUDIT ===" -ForegroundColor Cyan
npm run h48.4:attendance:audit
if ($LASTEXITCODE -ne 0) { throw "STOP - H48.4 static audit failed." }

Write-Host ""
Write-Host "=== H48.4 TARGETED + TIME REGRESSION TESTS ===" -ForegroundColor Cyan
npm run test:h48.4
if ($LASTEXITCODE -ne 0) { throw "STOP - H48.4 targeted tests failed." }
...
Write-Host " H48.4 ATTENDANCE VISIBILITY + TIMESTAMP VALIDATION: PASS" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
Write-Host "H48.4 audit: PASS" -ForegroundColor Green
Write-Host "H48.4 targeted/time tests: PASS" -ForegroundColor Green
