...
const candidates=[supplied,path.join(process.cwd(),'artifacts','v7-31-certification-ledger.json'),path.join(process.env.TEMP||process.env.TMP||'/tmp','opsiqo-v7-31-certification-ledger.json')].filter(Boolean);
const ledgerPath=candidates.find(p=>fs.existsSync(p));
if(!ledgerPath){console.log('V7.31 certification summary: no sanitized ledger exists yet.');process.exit(0)}
let data;try{data=JSON.parse(fs.readFileSync(ledgerPath,'utf8'))}catch{console.log('V7.31 certification summary: ledger exists but is not valid JSON.');process.exit(1)}
...
console.log('\nOPSIQO ONE V7.31 CERTIFICATION SUMMARY');
