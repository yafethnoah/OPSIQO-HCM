...
const runnerPath=path.join(root,'RUN_OPSIQO_ONE_V7_28_VALIDATION.ps1');
if(fs.existsSync(runnerPath)){
 ...
 add('runner-ledger',/v7-28-certification-ledger\.json/i.test(runner)?'pass':'fail','runner must persist a sanitized certification gate ledger');
 add('runner-env-snapshot',/Capture-V728Environment/.test(runner)&&/Restore-V728Environment/.test(runner)?'pass':'fail','runner must snapshot and restore environment variables around browser/emulator UAT');
 add('runner-env-restore-auth',/AuthEnvSnapshot\s*=\s*Capture-V728Environment/.test(runner)&&/Restore-V728Environment \$AuthEnvSnapshot/.test(runner)?'pass':'fail','authenticated UAT must restore the pre-existing shell environment');
 add('runner-env-restore-public',/PublicEnvSnapshot\s*=\s*Capture-V728Environment/.test(runner)&&/Restore-V728Environment \$PublicEnvSnapshot/.test(runner)?'pass':'fail','public browser smoke must restore OPSIQO_A11Y_BASE_URL');
 add('runner-safe-failure-summary',/opsiqo85-v7-28-certification-summary\.mjs/.test(runner)?'pass':'fail','runner must print a dependency-free sanitized summary on failure and completion');
...
}else add('v7-28-runner','fail','RUN_OPSIQO_ONE_V7_28_VALIDATION.ps1 must be packaged');
...
if(process.platform==='win32')add('windows-path-length',root.length<=160?'pass':'warn',`project path length=${root.length}; use a short extraction path such as D:\opsiqo\v728 when possible`);
...
for(const p of [8080,9099,9199,31732,31733])add(`port-${p}`,await portFree(p)?'pass':'fail',`127.0.0.1:${p} must be available for isolated certification`);
...
console.log(`\nV7.28 certification preflight: ${failed.length?'FAIL':'PASS'} (${checks.filter(x=>x.status==='pass').length}/${checks.length} checks passed${checks.some(x=>x.status==='warn')?', warnings present':''}).`);
