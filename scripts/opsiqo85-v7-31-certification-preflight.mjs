...
const runnerPath=path.join(root,'RUN_OPSIQO_ONE_V7_31_VALIDATION.ps1');
if(fs.existsSync(runnerPath)){
 ...
 add('runner-ledger',/v7-31-certification-ledger\.json/i.test(runner)?'pass':'fail','runner must persist a sanitized certification gate ledger');
 add('runner-env-snapshot',/Capture-V731Environment/.test(runner)&&/Restore-V731Environment/.test(runner)?'pass':'fail','runner must snapshot and restore environment variables around browser/emulator UAT');
 add('runner-env-restore-auth',/AuthEnvSnapshot\s*=\s*Capture-V731Environment/.test(runner)&&/Restore-V731Environment \$AuthEnvSnapshot/.test(runner)?'pass':'fail','authenticated UAT must restore the pre-existing shell environment');
 add('runner-env-restore-public',/PublicEnvSnapshot\s*=\s*Capture-V731Environment/.test(runner)&&/Restore-V731Environment \$PublicEnvSnapshot/.test(runner)?'pass':'fail','public browser smoke must restore OPSIQO_A11Y_BASE_URL');
 add('runner-safe-failure-summary',/opsiqo85-v7-31-certification-summary\.mjs/.test(runner)?'pass':'fail','runner must print a dependency-free sanitized summary on failure and completion');
...
}else add('v7-31-runner','fail','RUN_OPSIQO_ONE_V7_31_VALIDATION.ps1 must be packaged');
...
console.log(`\nV7.31 certification preflight: ${failed.length?'FAIL':'PASS'} (${checks.filter(x=>x.status==='pass').length}/${checks.length} checks passed${checks.some(x=>x.status==='warn')?', warnings present':''}).`);
