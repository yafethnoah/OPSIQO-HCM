...
if(exists('scripts/opsiqo-clean-release-audit.mjs')){
 const cleanAudit=fs.readFileSync(path.join(root,'scripts/opsiqo-clean-release-audit.mjs'),'utf8');
 add('clean-release-local-env-overlay-policy',cleanAudit.includes("--certification-local-env-overlay")&&cleanAudit.includes("allowedCertificationOverlay = '.env.local'")?'pass':'fail','certification-only clean-release mode must permit root .env.local without reading its contents');
}
add('root-dotenv-disallowed',!exists('.env')?'pass':'fail','root .env is not permitted during certification; use .env.local for local runtime configuration');
add('root-dotenv-local-overlay','pass',exists('.env.local')?'root .env.local detected as certification-only local runtime overlay; contents are not inspected':'root .env.local not present');
...
const runnerPath=path.join(root,'RUN_OPSIQO_ONE_V7_32_VALIDATION.ps1');
if(fs.existsSync(runnerPath)){
 ...
 add('runner-ledger',/v7-32-certification-ledger\.json/i.test(runner)?'pass':'fail','runner must persist a sanitized certification gate ledger');
 add('runner-env-snapshot',/Capture-V732Environment/.test(runner)&&/Restore-V732Environment/.test(runner)?'pass':'fail','runner must snapshot and restore environment variables around browser/emulator UAT');
 add('runner-env-restore-auth',/AuthEnvSnapshot\s*=\s*Capture-V732Environment/.test(runner)&&/Restore-V732Environment \$AuthEnvSnapshot/.test(runner)?'pass':'fail','authenticated UAT must restore the pre-existing shell environment');
 add('runner-env-restore-public',/PublicEnvSnapshot\s*=\s*Capture-V732Environment/.test(runner)&&/Restore-V732Environment \$PublicEnvSnapshot/.test(runner)?'pass':'fail','public browser smoke must restore OPSIQO_A11Y_BASE_URL');
 add('runner-safe-failure-summary',/opsiqo85-v7-32-certification-summary\.mjs/.test(runner)?'pass':'fail','runner must print a dependency-free sanitized summary on failure and completion');
 add('runner-local-env-overlay',/opsiqo-clean-release-audit\.mjs\s+--certification-local-env-overlay/.test(runner)?'pass':'fail','runner must use certification-only root .env.local overlay mode for the pre-install clean-release gate');
...
}else add('v7-32-runner','fail','RUN_OPSIQO_ONE_V7_32_VALIDATION.ps1 must be packaged');
...
console.log(`\nV7.32 certification preflight: ${failed.length?'FAIL':'PASS'} (${checks.filter(x=>x.status==='pass').length}/${checks.length} checks passed${checks.some(x=>x.status==='warn')?', warnings present':''}).`);
