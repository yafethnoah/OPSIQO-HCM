...
const candidates=[supplied,path.join(process.cwd(),'artifacts','v7-32-certification-ledger.json'),path.join(process.env.TEMP||process.env.TMP||'/tmp','opsiqo-v7-32-certification-ledger.json')].filter(Boolean);
const ledgerPath=candidates.find(p=>fs.existsSync(p));
if(!ledgerPath){console.log('V7.32 certification summary: no sanitized ledger exists yet.');process.exit(0)}
let data;try{const raw=fs.readFileSync(ledgerPath,'utf8').replace(/^\uFEFF/,'');data=JSON.parse(raw)}catch{console.log('V7.32 certification summary: ledger exists but is not valid JSON.');process.exit(1)}
...
console.log('\nOPSIQO ONE V7.32 CERTIFICATION SUMMARY');
