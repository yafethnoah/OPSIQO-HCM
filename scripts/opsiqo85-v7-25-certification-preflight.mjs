...
const runnerPath=path.join(root,'RUN_OPSIQO_ONE_V7_25_VALIDATION.ps1');
if(fs.existsSync(runnerPath)){
 ...
 add('runner-ledger',/v7-25-certification-ledger\.json/i.test(runner)?'pass':'fail','runner must persist a sanitized certification gate ledger');
...
}else add('v7-25-runner','fail','RUN_OPSIQO_ONE_V7_25_VALIDATION.ps1 must be packaged');
...
if(process.platform==='win32')add('windows-path-length',root.length<=160?'pass':'warn',`project path length=${root.length}; use a short extraction path such as D:\opsiqo\v725 when possible`);
...
console.log(`\nV7.25 certification preflight: ${failed.length?'FAIL':'PASS'} (${checks.filter(x=>x.status==='pass').length}/${checks.length} checks passed${checks.some(x=>x.status==='warn')?', warnings present':''}).`);
