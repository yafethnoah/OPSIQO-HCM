...
function Step([string]$Name,[scriptblock]$Action) {
  ...
  Write-Host " OPSIQO ONE V7.18 - $Name" -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  & $Action
...
function Clear-V718DemoEnvironment {
...
Set-Location $Root
Write-Host 'OPSIQO ONE v7.18 local certification' -ForegroundColor Cyan
...
Step 'V7.18 UAT Accessibility Translation Certification audit' { npm run opsiqo85:opsiqo-one-v7.18:audit }
Step 'V7.18 translation inventory snapshot verification' { npm run opsiqo85:v7.18:translation-inventory:verify }
...
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.18 - Built-app public browser accessibility smoke' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
...
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.18 - Authenticated emulator-backed accessibility UAT' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  $DemoServer=Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31718' -PassThru -WindowStyle Hidden
  $Ready=$false
  foreach($Attempt in 1..90){try{$Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31718/home' -UseBasicParsing -TimeoutSec 2;if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break}}catch{Start-Sleep -Seconds 1}}
  if(-not $Ready){throw 'Authenticated demo server did not become reachable on port 31718.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31718'
  npm run opsiqo85:v7.18:browser-a11y-auth
...
Write-Host ' OPSIQO ONE V7.18 CERTIFICATION PASS' -ForegroundColor Green
Write-Host ('=' * 76) -ForegroundColor Green
