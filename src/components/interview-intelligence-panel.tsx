...
export function InterviewIntelligencePanel({interviews,onChanged}:{interviews:Interview[];onChanged:()=>Promise<void>|void}){
 const [interviewId,setInterviewId]=useState(""); const [kit,setKit]=useState<Kit|null>(null); const [summary,setSummary]=useState<PanelSummary|null>(null); const [error,setError]=useState(""); const [notice,setNotice]=useState(""); const [busy,setBusy]=useState(false);
 const interview=useMemo(()=>interviews.find(i=>i.id===interviewId),[interviews,interviewId]);
 async function loadKit(id:string){if(!id){setKit(null);setSummary(null);return;}setError("");try{const r=await apiFetch<{data:{kit:Kit;panelSummary:PanelSummary}}>(`/api/organizations/${activeOrgId()}/recruiting/interviews/${id}/kit`);setKit(r.data.kit);setSummary(r.data.panelSummary);}catch(e){setError(e instanceof Error?e.message:"Unable to load interview kit.");}}
 useEffect(()=>{void loadKit(interviewId)},[interviewId]);
 async function kitAction(action:"lock"|"regenerate"){if(!interviewId)return;setBusy(true);setError("");setNotice("");try{await apiFetch(`/api/organizations/${activeOrgId()}/recruiting/interviews/${interviewId}/kit`,{method:"PATCH",body:JSON.stringify({action})});setNotice(action==="lock"?"Interview kit locked for consistent use.":"Interview kit regenerated for human review.");await loadKit(interviewId);}catch(e){setError(e instanceof Error?e.message:"Interview kit update failed.");}finally{setBusy(false)}}
 async function submitScore(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!interviewId||!kit)return;const f=new FormData(e.currentTarget);const ratings=kit.questions.filter(q=>q.type!=="candidate_questions").map(q=>({criterion:q.competency,rating:Number(f.get(`rating_${q.id}`)||3),evidence:String(f.get(`evidence_${q.id}`)||"")||undefined}));setBusy(true);setError("");setNotice("");try{await apiFetch(`/api/organizations/${activeOrgId()}/recruiting/interviews/${interviewId}/scorecards`,{method:"POST",body:JSON.stringify({recommendation:f.get("recommendation"),ratings,overallComment:f.get("overallComment")||undefined})});setNotice("Immutable evidence-based scorecard submitted. OPSIQO has not made an employment decision.");e.currentTarget.reset();await loadKit(interviewId);await onChanged();}catch(e){setError(e instanceof Error?e.message:"Unable to submit scorecard.");}finally{setBusy(false)}}
 return <section className="card stack">
   ...
   <label className="field"><span>Interview</span><select className="input" value={interviewId} onChange={e=>setInterviewId(e.target.value)} disabled={!interviews.length}><option value="">{interviews.length?"Select interview":"No interviews available"}</option>{interviews.map(i=><option key={i.id} value={i.id}>{i.interviewType} · {new Date(i.scheduledAt).toLocaleString()}</option>)}</select></label>
   {kit&&<>
