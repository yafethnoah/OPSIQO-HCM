...
export function RecruitingWorkspace() {
  ...
  async function scheduleInterview(e: FormEvent<HTMLFormElement>) {
    ...
    setNotice("");
    try {
      const response = await apiFetch<{ data: Interview & { interviewKit?: { id: string; status: string; questions: unknown[] } } }>(
        `/api/organizations/${activeOrgId()}/recruiting/interviews`,
        {
          ...
          body: JSON.stringify({
            ...
            interviewerWorkerIds: f.getAll("interviewerWorkerIds").map((v) => String(v)).filter(Boolean),
            meetingUrl: f.get("meetingUrl") || undefined,
          }),
        },
      );
      setPreferredInterviewId(response.data.id);
      setNotice(response.data.interviewKit
        ? `Interview scheduled and structured interview kit prepared (${response.data.interviewKit.questions.length} questions). Review and lock it before use.`
        : "Interview scheduled. Preparing the structured interview kit…");
      form.reset();
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Unable to schedule interview.");
    }
  }
    ...
    <div ref={translationRoot} className="stack">
      ...
      {can("recruiting.interview") && (
        <div className="grid2">
          ...
          <InterviewIntelligencePanel interviews={interviews} preferredInterviewId={preferredInterviewId} onChanged={load} />
        </div>
      )}
      {can("recruiting.offer") && (
        <div className="grid2">
