...
const submission=z.object({firstName:z.string().trim().min(1).max(100),lastName:z.string().trim().min(1).max(100),email:z.string().email(),phone:z.string().max(50).optional(),location:z.string().max(160).optional(),linkedinUrl:z.string().max(500).optional(),portfolioUrl:z.string().max(500).optional(),githubUrl:z.string().max(500).optional(),socialMediaUrl:z.string().max(500).optional(),headline:z.string().max(240).optional(),summary:z.string().max(5000).optional(),skills:z.array(z.string().max(160)).max(150).default([]),certifications:z.array(z.string().max(240)).max(80).default([]),education:z.array(z.string().max(500)).max(80).default([]),yearsOfExperience:z.number().min(0).max(80).optional(),screeningAnswers:z.record(z.string(),z.union([z.string(),z.number(),z.boolean()])).default({}),coverLetterText:z.string().max(100000).optional(),candidateStatement:z.string().max(5000).optional(),consent:z.literal(true),accuracyConfirmed:z.literal(true),talentPoolConsent:z.boolean().default(false)});
