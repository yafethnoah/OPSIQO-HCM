...
const RESUME_PROMPT_VERSION='RECRUITING_RESUME_PARSE_V4_ASSURANCE';
...
function normalizeResume(raw:any,text:string,provider:string,model:string,fileName?:string):ParsedResumeProfile{
 ...
 const supportedList=(value:any)=>arr(value,120).filter((entry:string)=>{const words=entry.toLowerCase().match(/[a-z0-9+#.]{2,}/g)||[];if(!words.length)return false;const source=evidenceText.toLowerCase();const hit=words.filter(w=>source.includes(w)).length;return hit/words.length>=0.6});
...
export async function governedResumeParse(actor:ActorContext,input:{name:string;mimeType:string;bytes:Buffer;text:string}){
 ...
 const grounded=(v:any,max=1000)=>{const text=String(v||'').trim();if(!text)return undefined;const words=text.toLowerCase().match(/[a-z0-9+#.]{2,}/g)||[];if(!words.length)return undefined;const hit=words.filter(w=>src.includes(w)).length;return hit/words.length>=0.55?text.slice(0,max):undefined};
 ...
 const languageRecords=arr(raw?.languageRecords).slice(0,30).map((x:any)=>({language:grounded(x?.language,120)||'',proficiency:grounded(x?.proficiency,120)})).filter((x:any)=>x.language);
