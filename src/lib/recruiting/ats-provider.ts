...
const RESUME_PROMPT_VERSION='RECRUITING_RESUME_PARSE_V15_RECORD_BOUND_EDUCATION_PROVENANCE';
...
function repairDefectVector(value:any,source:string){
 ...
 const seenEducationDates=new Map<string,string>();
 for(const row of list(value?.educationHistory)){
   const key=canonical(row?.degree)+'|'+canonical(row?.institution);
   const date=canonical(row?.graduationDate||row?.endDate);
   if(date){
     const owner=seenEducationDates.get(date);
     if(owner&&owner!==key)defects+=4;
     else seenEducationDates.set(date,key);
   }
 }
...
export async function governedStructuredResumeRepair(
 ...
 const prompt=[
   ...
   'Merge duplicate partial and complete records. Remove heading/narrative/action-sentence contamination. Canonicalize duplicate volunteer organizations to the shortest source-grounded organization name. EDUCATION PROVENANCE: assign every education date/status/location only from the same bounded degree/institution block; never propagate an expected date from one education record to another; one evidence span cannot populate multiple unrelated records. SOURCE COMPLETENESS: recover expected graduation/completion dates even when PDF extraction wraps Expected and the date across separate lines; mark in-progress education completed=false; recover source-supported certifications/licences. Never claim structural completeness by silently omitting a source-supported fact.',
