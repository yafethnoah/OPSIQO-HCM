...
export async function createInterview(actor: ActorContext, raw: unknown) {
  ...
  const enhanced=await governedInterviewQuestionEnhancement(actor,{requisition,candidate,review,questions:baseline}); const questions=enhanced||baseline;
  const kit:InterviewKit={id:interview.id,interviewId:interview.id,applicationId:application.id,requisitionId:requisition.id,candidateId:candidate.id,version:1,status:'draft',generationMode:enhanced?'governed_ai':'deterministic',questions,decisionBoundary:'Interview questions and scorecards support human evaluation only. OPSIQO does not automatically hire, reject, advance, or infer protected characteristics from interview data.',createdBy:actor.uid,createdAt:timestamp,updatedAt:timestamp};
  const audit = buildAudit(actor, { action: 'interview.create', entityType: 'interview', entityId: interview.id, after: interview, metadata:{interviewKitVersion:1,questionCount:questions.length,generationMode:kit.generationMode} }); const stageEvent = buildDomainEvent(actor, 'application.stage_changed', 'application', application.id, { from: application.stage, to: 'interview', requisitionId: application.requisitionId, candidateId: application.candidateId, source: 'interview.create' }); const batch = db.batch(); batch.create(db.doc(`organizations/${actor.orgId}/interviews/${interview.id}`), interview); batch.create(db.doc(`organizations/${actor.orgId}/interviewKits/${interview.id}`), kit); batch.create(db.doc(`organizations/${actor.orgId}/auditLogs/${audit.id}`), audit); if (application.stage !== 'interview') { batch.update(db.doc(`organizations/${actor.orgId}/applications/${application.id}`), { stage: 'interview', updatedAt: timestamp }); batch.create(db.doc(`organizations/${actor.orgId}/domainEvents/${stageEvent.id}`), stageEvent); } await batch.commit(); return {...interview,interviewKit:kit};
}

export async function getInterviewKit(actor:ActorContext,interviewId:string){const db=adminDb();const interviewSnap=await db.doc(`organizations/${actor.orgId}/interviews/${interviewId}`).get();if(!interviewSnap.exists)throw new ApiError(404,'Interview not found.','interview_not_found');const interview=interviewSnap.data() as Interview;if(!hrRoles.has(actor.role)&&!interview.interviewerUids.includes(actor.uid))throw new ApiError(403,'Interview is outside your assigned scope.','interview_scope');const kitSnap=await db.doc(`organizations/${actor.orgId}/interviewKits/${interviewId}`).get();if(!kitSnap.exists)throw new ApiError(404,'Interview kit not found.','interview_kit_not_found');const scorecards=await db.collection(`organizations/${actor.orgId}/scorecards`).where('interviewId','==',interviewId).get();const rows=scorecards.docs.map(d=>d.data() as InterviewScorecard);const evaluatorAverages=rows.map(r=>r.ratings.length?r.ratings.reduce((sum,x)=>sum+x.rating,0)/r.ratings.length:0).filter(x=>x>0);const avg=evaluatorAverages.length?evaluatorAverages.reduce((a,b)=>a+b,0)/evaluatorAverages.length:undefined;const spread=evaluatorAverages.length>1?Math.max(...evaluatorAverages)-Math.min(...evaluatorAverages):undefined;const recommendationCounts:Record<string,number>={};for(const r of rows)recommendationCounts[r.recommendation]=(recommendationCounts[r.recommendation]||0)+1;const summary:InterviewPanelSummary={interviewId,scorecardCount:rows.length,averageRating:avg==null?undefined:Math.round(avg*100)/100,recommendationCounts,varianceWarning:(spread||0)>=3,ratingSpread:spread,decisionBoundary:'Panel analytics highlight consistency and variance; they do not make the employment decision.'};return{kit:kitSnap.data() as InterviewKit,panelSummary:summary,scorecards:rows};}

export async function updateInterviewKit(actor:ActorContext,interviewId:string,raw:unknown){const input=interviewKitUpdateSchema.parse(raw);const db=adminDb();const interviewSnap=await db.doc(`organizations/${actor.orgId}/interviews/${interviewId}`).get();if(!interviewSnap.exists)throw new ApiError(404,'Interview not found.','interview_not_found');const interview=interviewSnap.data() as Interview;if(!hrRoles.has(actor.role)&&!interview.interviewerUids.includes(actor.uid))throw new ApiError(403,'Interview is outside your assigned scope.','interview_scope');const canManageKit=hrRoles.has(actor.role)||actor.permissions.includes('recruiting.manage' as any)||actor.permissions.includes('recruiting.manage.team' as any);if(!canManageKit)throw new ApiError(403,'Recruiting management permission is required to regenerate or lock an interview kit.','interview_kit_manage_required');const kitRef=db.doc(`organizations/${actor.orgId}/interviewKits/${interviewId}`);const kitSnap=await kitRef.get();if(!kitSnap.exists)throw new ApiError(404,'Interview kit not found.','interview_kit_not_found');const before=kitSnap.data() as InterviewKit;if(before.status==='locked'&&input.action==='lock')return before;if(before.status==='locked'&&input.action==='regenerate')throw new ApiError(409,'Locked interview kits cannot be regenerated.','interview_kit_locked');const timestamp=now();let after=before;if(input.action==='lock'){after={...before,questions:input.questions||before.questions,status:'locked',lockedBy:actor.uid,lockedAt:timestamp,updatedAt:timestamp};}else{const bundle=await applicationBundle(actor,interview.applicationId);let review:any=null;const latestReviewId=(bundle.application as any).atsLatestReviewId as string|undefined;if(latestReviewId){const rs=await db.doc(`organizations/${actor.orgId}/atsResumeReviews/${latestReviewId}`).get();if(rs.exists)review=rs.data();}const deterministic=buildDeterministicInterviewKit({requisition:bundle.requisition,candidate:bundle.candidate,review,interviewType:interview.interviewType});const enhanced=await governedInterviewQuestionEnhancement(actor,{requisition:bundle.requisition,candidate:bundle.candidate,review,questions:deterministic});after={...before,version:before.version+1,questions:enhanced||deterministic,generationMode:enhanced?'governed_ai':'deterministic',updatedAt:timestamp};}
 const audit=buildAudit(actor,{action:`interview.kit.${input.action}`,entityType:'interviewKit',entityId:interviewId,before,after,metadata:{version:after.version,questionCount:after.questions.length}});const batch=db.batch();batch.set(kitRef,after);batch.create(db.doc(`organizations/${actor.orgId}/auditLogs/${audit.id}`),audit);if(input.action==='lock'){const bankRef=db.doc(`organizations/${actor.orgId}/interviewQuestionBanks/${interview.requisitionId}`);const bankSnap=await bankRef.get();const bankVersion=Number(bankSnap.data()?.version||0)+1;batch.set(bankRef,{requisitionId:interview.requisitionId,version:bankVersion,status:'approved_locked_core',sourceInterviewKitId:interviewId,sourceInterviewKitVersion:after.version,coreQuestions:after.questions.filter(q=>q.standardized),updatedAt:timestamp,updatedBy:actor.uid},{merge:true});}await batch.commit();return after;}
