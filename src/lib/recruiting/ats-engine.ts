...
function fallbackEducationLines(sourceText:string){const cue=/\b(?:bachelor(?:'s)?|master(?:'s)?|doctorate|ph\.?d\.?|b\.?sc\.?|m\.?sc\.?|b\.?a\.?|m\.?a\.?|mba|doctor\s+of\s+pharmacy|pharm\.?d\.?|pharmacy\s+degree|diploma|degree)\b/i;return sourceText.split(/\r?\n/).map(x=>x.replace(/\s+/g,' ').trim()).filter(x=>x.length>=4&&x.length<=240&&cue.test(x)).slice(0,20);}
