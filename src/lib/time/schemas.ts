...
export const timePolicySchema = z.object({
  ...
  minBetweenShiftsHours:z.number().min(0).max(24).default(8), weeklyRestHours:z.number().min(0).max(168).default(24), mealBreakAfterHours:z.number().positive().max(12).default(5), mealBreakMinutes:z.number().int().min(0).max(180).default(30), roundingMinutes:z.number().int().min(1).max(60).default(15),
  complianceMode:z.enum(['advisory','enforce']).default('advisory'), captureGeolocation:z.boolean().default(false), electronicMonitoringPolicyId:z.string().optional(), enabled:z.boolean().default(true),
}).superRefine((v,ctx)=>{if(v.captureGeolocation&&!v.electronicMonitoringPolicyId)ctx.addIssue({code:'custom',message:'A governed electronic-monitoring policy is required before geolocation can be enabled.',path:['electronicMonitoringPolicyId']});});
...
export const clockSchema=z.object({action:z.enum(['clock_in','clock_out']),breakMinutes:z.number().int().min(0).max(720).optional(),note:z.string().max(500).optional()});
