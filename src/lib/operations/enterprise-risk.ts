...
export function riskStatus(score:number, highCritical=0, overdue=0):EnterpriseRiskStatus {
...
export function enterpriseIndex(nodes:EnterpriseRiskNode[]){
  const totalWeight=nodes.reduce((n,x)=>n+x.weight,0)||1;
  const weighted=nodes.reduce((n,x)=>n+bounded(x.readinessScore)*x.weight,0)/totalWeight;
  const criticalPenalty=nodes.reduce((n,x)=>n+(x.status==='critical'?4:x.status==='high'?2:0),0);
  const overduePenalty=Math.min(12,nodes.reduce((n,x)=>n+x.overdue,0));
  const score=bounded(weighted-criticalPenalty-overduePenalty*.35);
  return {score,level:readinessLevel(score)};
}
