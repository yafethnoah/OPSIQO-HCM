...
import type {
  ...
  PositionCompensationProfile,
  SalaryBand,
  WorkerCompensationRecord,
...
function canRead(a: ActorContext) {
  return (
    a.permissions.includes('compensation.read') ||
    a.permissions.includes('compensation.manage') ||
    a.permissions.includes('compensation.approve')
  );
}
...
export async function getCompensationReadiness(a: ActorContext): Promise<CompensationReadiness> {
  ...
  const [
    ...
    all<Assignment>(`${base}/assignments`),
    all<Position>(`${base}/positions`),
    all<WorkerCompensationRecord>(`${base}/workerCompensationCurrent`),
    all<PositionCompensationProfile>(`${base}/positionCompensationProfiles`),
    all<SalaryBand>(`${base}/salaryBands`),
    all<PayrollWorkerProfile>(`${base}/payrollWorkerProfiles`),
    all<any>(`${base}/requisitions`),
  ...
  const rows = active.map((w) => {
    ...
    if (comp && (!comp.standardHoursPerWeek || !comp.standardWeeksPerYear)) {
      issues.push('Pay conversion is using a legacy/policy schedule basis');
    }
    ...
    if (comp && position && !mappedBand) {
      ...
      const candidates = bands
        .filter((b) => {
          ...
          const low = normalizeBandAmount(
            b.payBasis,
            b.min,
            b.standardHoursPerWeek,
            b.standardWeeksPerYear,
          ).annualPay;
          const high = normalizeBandAmount(
            b.payBasis,
            b.max,
            b.standardHoursPerWeek,
            b.standardWeeksPerYear,
        ...
        .map((b) => {
          ...
          if (b.location && position.location && b.location.toLowerCase() === position.location.toLowerCase()) {
            confidence += 15;
          }
      ...
      if (candidates[0]) {
        suggestedBand = {
          bandId: candidates[0].b.id,
          code: candidates[0].b.code,
          name: candidates[0].b.name,
          confidence: candidates[0].confidence,
          reason:
            'Candidate band contains annualized pay in the same currency. HR review is required before mapping.',
        };
      }
    }
    ...
    return {
      ...
      issues,
      ready: issues.filter((i) => !i.includes('legacy/policy')).length === 0,
      suggestedBand,
  ...
  const missingPayrollProfile = rows.filter((x) =>
    x.issues.includes('Missing or disabled payroll profile'),
  ).length;
  ...
  return {
    ...
    requisitions: requisitions
      .filter((r) => !['closed', 'cancelled', 'filled'].includes(String(r.status || '').toLowerCase()))
      .slice(0, 100)
      .map((r) => ({
        id: r.id,
        title: r.title || r.name || r.positionTitle || r.id,
        status: r.status || 'open',
      })),
    aiSuggestions: [
      {
        title: 'Resolve compensation readiness',
        prompt:
          'Review compensation readiness for this organization. Prioritize missing compensation records, salary-band mappings, compensation-to-payroll synchronization, and payroll-profile blockers. Do not make pay decisions; propose governed administrative next steps with evidence.',
      },
      {
        title: 'Design salary architecture',
        prompt:
          'Review the current job families, levels, salary bands and position mappings. Identify architecture gaps and propose a sequenced configuration plan. Do not automatically change employee pay.',
      },
      {
        title: 'Prepare payroll readiness plan',
        prompt:
          'Review compensation-to-payroll readiness. Explain which worker records are blocked, synchronization status, and what authoritative data is required before payroll can run.',
      },
    ],
    generatedAt: now(),
  };
...
export async function prepareCompensationDrafts(a: ActorContext) {
  if (!a.permissions.includes('compensation.manage') && !a.permissions.includes('compensation.approve')) {
    throw new ApiError(403, 'Compensation management permission required.', 'forbidden');
  }
  ...
  const compIds = new Set(current.map((x) => x.workerId));
  const posMap = new Map(positions.map((x) => [x.id, x]));
  const profileMap = new Map(profiles.map((x) => [x.positionId, x]));
  const bandMap = new Map(bands.map((x) => [x.id, x]));
  const assignmentMap = new Map<string, Assignment>();
  for (const x of assignments.filter((x) => x.primary && !x.endDate)) assignmentMap.set(x.workerId, x);
  ...
  for (const w of workers.filter((x) => (x.status === 'active' || x.status === 'leave') && !compIds.has(x.id))) {
    const assignment = assignmentMap.get(w.id);
    const position = assignment ? posMap.get(assignment.positionId) : undefined;
    const profile = position ? profileMap.get(position.id) : undefined;
    const band = profile?.salaryBandId ? bandMap.get(profile.salaryBandId) : undefined;

    drafts.push({
      workerId: w.id,
      status: 'needs_review',
      source: 'h51.37.readiness',
      positionId: position?.id,
      positionTitle: position?.title,
      salaryBandId: band?.id,
      salaryBandCode: band?.code,
      currency: band?.currency || 'CAD',
      suggestedPayBasis: band?.payBasis,
      standardHoursPerWeek: band?.standardHoursPerWeek || 40,
      standardWeeksPerYear: band?.standardWeeksPerYear || 52,
      requiredFields: ['authoritative pay basis', 'authoritative pay amount', 'effective date', 'change reason'],
      note: 'Administrative setup draft only. OPSIQO does not infer or decide employee pay.',
      updatedBy: a.uid,
      updatedAt: now(),
    });
  }
  ...
  for (let i = 0; i < drafts.length; i += 300) {
    const b = db.batch();
    for (const x of drafts.slice(i, i + 300)) {
      b.set(db.doc(`${base}/compensationSetupDrafts/${x.workerId}`), x, { merge: true });
    }
    await b.commit();
  }
...
export async function preparePayrollProfileDrafts(a: ActorContext) {
  if (!a.permissions.includes('compensation.manage') && !a.permissions.includes('payroll.provider.manage')) {
    throw new ApiError(403, 'Compensation or payroll administration permission required.', 'forbidden');
  }
  ...
  const profileIds = new Set(profiles.map((x) => x.workerId));
  const compMap = new Map(current.map((x) => [x.workerId, x]));
  const drafts: any[] = [];
  ...
  for (const w of workers.filter((x) => x.status === 'active' || x.status === 'leave')) {
    ...
    drafts.push({
      ...
      monthlyPay: n.monthlyPay,
      annualPay: n.annualPay,
      standardHoursPerWeek: n.standardHoursPerWeek,
      standardWeeksPerYear: n.standardWeeksPerYear,
      requiredFields: [
        'province',
        'payPeriodsPerYear',
  ...
  for (let i = 0; i < drafts.length; i += 300) {
    const b = db.batch();
    for (const x of drafts.slice(i, i + 300)) {
      b.set(db.doc(`${base}/payrollProfileDrafts/${x.workerId}`), x, { merge: true });
    }
    await b.commit();
  }
...
export async function applySuggestedBand(a: ActorContext, workerId: string, bandId: string) {
  if (!a.permissions.includes('jobarchitecture.manage')) {
    throw new ApiError(403, 'Job architecture permission required.', 'forbidden');
  }

  const db = adminDb();
  const worker = await db.doc(`organizations/${a.orgId}/workers/${workerId}`).get();
  if (!worker.exists) throw new ApiError(404, 'Worker not found.', 'worker_not_found');

  const assignments = await db
    .collection(`organizations/${a.orgId}/assignments`)
    .where('workerId', '==', workerId)
    .where('primary', '==', true)
    .limit(20)
    .get();

  const assignment = assignments.docs
    .map((d) => d.data() as Assignment)
    .filter((x) => !x.endDate)
    .sort((x, y) => y.startDate.localeCompare(x.startDate))[0];

  if (!assignment) {
    throw new ApiError(409, 'Worker does not have a current primary assignment.', 'assignment_missing');
  }
