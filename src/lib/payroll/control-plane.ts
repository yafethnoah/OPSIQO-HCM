...
import{resolveWorkerPayDate}from'./pay-date';
...
export async function savePayrollWorkerProfile(a:ActorContext,workerId:string,raw:unknown){
  ...
  await ref.set(row);const syncRef=db.doc(`organizations/${a.orgId}/compensationPayrollSyncCurrent/${workerId}`),syncSnap=await syncRef.get();if(syncSnap.exists)await syncRef.set({status:'profile_reviewed',payrollProfileId:workerId,releaseDateRule:{payDateRule:x.payDateRule,payDateOffsetDays:x.payDateOffsetDays,payDayOfMonth:x.payDayOfMonth,weekendAdjustment:x.weekendAdjustment},updatedBy:a.uid,updatedAt:ts},{merge:true});const audit=buildAudit(a,{action:'payroll.worker_profile.save',entityType:'payrollWorkerProfile',entityId:workerId,before,after:{...row,paymentMethodRef:row.paymentMethodRef?'[payment-ref]':undefined}});
...
export async function createPayrollRun(a:ActorContext,raw:unknown){
  requireP(a,'payroll.calculate');const x=payrollRunCreateSchema.parse(raw);if(periodDays(x.periodStart,x.periodEnd)>366)throw new ApiError(409,'Payroll period cannot exceed 366 days.','invalid_period_range');const overrideIds=Object.keys(x.workerPayDates||{});if(overrideIds.length>2000)throw new ApiError(409,'Too many worker-specific payroll release dates.','pay_date_override_limit');if(x.workerIds?.length&&overrideIds.some(id=>!x.workerIds!.includes(id)))throw new ApiError(409,'Worker-specific release dates may only reference workers selected for the payroll run.','pay_date_worker_scope');
...
async function buildRunRows(a:ActorContext,run:PayrollRun){
  ...
  for(const w of selected){
    const p=profileMap[w.id];if(!p||!p.enabled)throw new ApiError(409,`Payroll profile is missing or disabled for ${w.displayName||w.id}.`,'payroll_profile_missing');const release=resolveWorkerPayDate(run,p,w.id),workerPayDate=release.payDate;
    ...
    const ads=adjustments.filter(x=>x.workerId===w.id&&x.status==='approved'&&!x.consumedByRunId&&((x.targetRunId&&x.targetRunId===run.id)||(!x.targetRunId&&x.payDate===workerPayDate)));
    ...
    const year=workerPayDate.slice(0,4),prior=statements.filter(s=>s.workerId===w.id&&s.status==='final'&&!s.reversedAt&&s.payDate.startsWith(year)&&s.payDate<workerPayDate);
...
export async function completePayrollRun(a:ActorContext,runId:string){
  requireP(a,'payroll.reconcile');
  const db=adminDb(),ref=db.doc(`organizations/${a.orgId}/payrollRuns/${runId}`),s=await ref.get();
  if(!s.exists)throw new ApiError(404,'Payroll run not found.','payroll_run_not_found');
  const run=s.data()as PayrollRun;
  if(run.status!=='reconciled'){
    if(run.status==='completed')return run;
    throw new ApiError(409,'Payroll run must reconcile without exceptions before completion.','invalid_payroll_state');
  }

  const{snapshots,calculations}=await loadRunData(a.orgId,run);
  const snapMap=Object.fromEntries(snapshots.map(x=>[x.workerId,x]));
  const prior=await all<PayStatement>(`organizations/${a.orgId}/payStatements`);
  const ts=now();

  for(let i=0;i<calculations.length;i+=150){
    const b=db.batch();
    for(const c of calculations.slice(i,i+150)){
      const x=snapMap[c.workerId],r=c.result;
      const payDate=r.payDate||x.payDate||run.payDate;
      const year=payDate.slice(0,4);
      const old=prior.filter(p=>p.workerId===c.workerId&&p.status==='final'&&!p.reversedAt&&p.payDate.startsWith(year)&&p.payDate<payDate);
      const ytd={
        gross:money(sum(old,p=>p.gross)+r.gross),
        cppOrQpp:money(sum(old,p=>p.cppOrQpp)+r.cppOrQpp),
        cpp2OrQpp2:money(sum(old,p=>p.cpp2OrQpp2)+r.cpp2OrQpp2),
        ei:money(sum(old,p=>p.ei)+r.ei),
        qpip:money(sum(old,p=>p.qpip)+r.qpip),
        federalTax:money(sum(old,p=>p.federalTax)+r.federalTax),
        provincialTax:money(sum(old,p=>p.provincialTax)+r.provincialTax),
        net:money(sum(old,p=>p.net)+r.net)
      };
      const statement:PayStatement={
        id:`${run.id}_${c.workerId}`,
        runId:run.id,
        workerId:c.workerId,
      ...
      b.set(db.doc(`organizations/${a.orgId}/payStatements/${statement.id}`),statement);
      for(const aid of x.adjustmentIds){
        b.set(db.doc(`organizations/${a.orgId}/payrollAdjustments/${aid}`),{status:'consumed',consumedByRunId:run.id,consumedAt:ts},{merge:true});
      }
    }
    await b.commit();
  }

  const releaseDates=Array.from(new Set(calculations.map(c=>c.result.payDate||snapMap[c.workerId]?.payDate||run.payDate))).sort();
  const after={...run,status:'completed' as const,completedBy:a.uid,completedAt:ts};
  await ref.set(after);
  const audit=buildAudit(a,{action:'payroll.run.complete',entityType:'payrollRun',entityId:runId,before:run,after});
  await db.doc(`organizations/${a.orgId}/auditLogs/${audit.id}`).create(audit);
  const ev=buildDomainEvent(a,'payroll.ready','payrollRun',run.id,{defaultPayDate:run.payDate,releaseDates,workers:run.totals?.workers||0});
  await db.doc(`organizations/${a.orgId}/domainEvents/${ev.id}`).create(ev);
  return after;
}
