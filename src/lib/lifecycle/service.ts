...
export async function lifecycleDashboard(actor:ActorContext):Promise<LifecycleDashboard>{
  ...
  return {metrics:[
    ...
    ...(integration?[{key:'integration',label:'Enterprise integrations',value:integration.metrics.readinessScore,helper:`${integration.metrics.activeConnectors} connector(s) · ${integration.metrics.activeRuntimeProfiles} runtime profile(s) · ${integration.metrics.openRuntimeCircuits} open circuit(s) · ${integration.metrics.openDeadLetters} dead letter(s)`,href:'/integrations'}]:[]),
    ...(identity?[{key:'identity',label:'Identity & access assurance',value:identity.metrics.readinessScore,helper:`${identity.metrics.activeProviders} SSO provider(s) · ${identity.metrics.pendingAccessApprovals} access approval(s) · ${identity.metrics.orphanIdentityAccounts} orphan identity account(s)`,href:'/identity'}]:[]),
    ...(securityOps?[{key:'security_ops',label:'HCM security & zero-trust assurance',value:securityOps.metrics.readinessScore,helper:`${securityOps.metrics.openIncidents} open incident(s) · ${securityOps.metrics.pendingPrivilegedAccess} privileged access request(s) · ${securityOps.metrics.privilegedMfaGaps} privileged MFA gap(s)`,href:'/security-operations'}]:[]),
    ...(platform?[{key:'platform_reliability',label:'Platform reliability & supply chain',value:platform.metrics.readinessScore,helper:`${platform.metrics.criticalHighSupplyChainFindings} supply-chain finding(s) · ${platform.metrics.backupEvidenceOverdue} backup evidence gap(s) · ${platform.metrics.sloBreaches} SLO breach(es)`,href:'/platform-reliability'}]:[]),
    ...
    {key:'complianceGaps',label:'Compliance gaps',value:complianceGaps,helper:`${Number(compliance.metrics?.complianceRate||100)}% compliance rate`,href:'/compliance'},
