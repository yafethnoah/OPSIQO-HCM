...
export default function SignInPage(){
  ...
  async function finishUser(user:User){
    ...
    try{
      const organizations=await apiFetch<{data:{orgId:string}[]}>('/api/me/organizations',{orgContext:'omit'});
      if(!resolvedOrgId&&organizations.data.length===1)resolvedOrgId=organizations.data[0]!.orgId;
    }catch{
      // Organization discovery is retried by setup/home when needed.
    }
    if(resolvedOrgId)setActiveOrgId(resolvedOrgId);
    router.push(await signedInLanding(resolvedOrgId));
  }
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      if(!beginMfaChallenge(e))setError(friendlyFirebaseAuthError(e,'signin','Sign-in failed.'));
  ...
  async function guest(){if(!orgId||!policy.guestAccessEnabled)return;setBusy(true);setError('');try{await signInAnonymously(firebaseAuth());setActiveOrgId(orgId);router.push('/organization')}catch(e){setError(friendlyFirebaseAuthError(e,'guest','Guest access could not be started.'))}finally{setBusy(false)}}
  async function sso(p:PublicProvider){try{setError('');const provider=p.protocol==='saml'?new SAMLAuthProvider(p.firebaseProviderId):new OAuthProvider(p.firebaseProviderId);await signInWithRedirect(firebaseAuth(),provider)}catch(e){setError(friendlyFirebaseAuthError(e,'sso','SSO sign-in could not start.'))}}
