...
import { createSecondaryAssignment } from '@/lib/hr/service';
...
export async function POST(request: Request, context: { params: Promise<{ orgId: string; workerId: string }> }) {
  try {
    ...
    return NextResponse.json({ data: await createSecondaryAssignment(actor, workerId, await request.json()) }, { status: 201 });
  } catch (error) {
