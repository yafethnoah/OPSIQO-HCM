...
$script:CertificationLedger = Join-Path ([System.IO.Path]::GetTempPath()) 'opsiqo-v7-27-certification-ledger.json'
...
function Step([string]$Name,[scriptblock]$Action) {
  ...
  Write-Host " OPSIQO ONE V7.27 - $Name" -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  try {
    ...
    try { node scripts/opsiqo85-v7-27-certification-summary.mjs $script:CertificationLedger } catch {}
    throw
  }
}
...
function Capture-V727Environment([string[]]$Names) {
  $Snapshot = @{}
...
function Restore-V727Environment($Snapshot) {
...
function Clear-V727DemoEnvironment {
...
Write-Host 'SAFE - sanitized gate ledger: artifacts\v7-27-certification-ledger.json' -ForegroundColor Yellow
...
Step 'Certification machine preflight' { node scripts/opsiqo85-v7-27-certification-preflight.mjs }
...
$script:CertificationLedger = Join-Path $Root 'artifacts\v7-27-certification-ledger.json'
Save-CertificationLedger
Step 'Locked dependency installation' { npm ci --no-audit --no-fund }
Step 'Locked toolchain post-install preflight' { node scripts/opsiqo85-v7-27-certification-preflight.mjs --post-install }
Step 'V7.27 Certification Environment & Translation Closure audit' { npm run opsiqo85:opsiqo-one-v7.27:audit }
Step 'V7.27 translation inventory verification' { npm run opsiqo85:v7.27:translation-inventory:verify }
...
$PublicServer = $null
$PublicEnvSnapshot = Capture-V727Environment @('OPSIQO_A11Y_BASE_URL')
$PublicStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.27 - Built-app public browser accessibility smoke' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  $PublicServer = Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31730' -PassThru -WindowStyle Hidden
  $Ready = $false
  foreach ($Attempt in 1..90) { try { $Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31730/signin' -UseBasicParsing -TimeoutSec 2; if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break} } catch { Start-Sleep -Seconds 1 } }
  if(-not $Ready){throw 'Built production server did not become reachable on port 31730.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31730'
  ...
  Restore-V727Environment $PublicEnvSnapshot
...
$AuthEnvSnapshot = Capture-V727Environment $script:V727DemoEnvironmentNames
$AuthStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.27 - Authenticated emulator-backed accessibility UAT' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  $DemoServer=Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31731' -PassThru -WindowStyle Hidden
  $Ready=$false
  foreach($Attempt in 1..90){try{$Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31731/home' -UseBasicParsing -TimeoutSec 2;if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break}}catch{Start-Sleep -Seconds 1}}
  if(-not $Ready){throw 'Authenticated demo server did not become reachable on port 31731.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31731'
  npm run opsiqo85:v7.27:browser-a11y-auth
  ...
  Restore-V727Environment $AuthEnvSnapshot
}
...
Save-CertificationLedger
node scripts/opsiqo85-v7-27-certification-summary.mjs $script:CertificationLedger
Write-Host ''
Write-Host ('=' * 76) -ForegroundColor Green
Write-Host ' OPSIQO ONE V7.27 CERTIFICATION PASS' -ForegroundColor Green
Write-Host ('=' * 76) -ForegroundColor Green
