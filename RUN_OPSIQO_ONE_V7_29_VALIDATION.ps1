...
$script:CertificationLedger = Join-Path ([System.IO.Path]::GetTempPath()) 'opsiqo-v7-29-certification-ledger.json'
...
function Step([string]$Name,[scriptblock]$Action) {
  ...
  Write-Host " OPSIQO ONE V7.29 - $Name" -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  try {
    ...
    try { node scripts/opsiqo85-v7-29-certification-summary.mjs $script:CertificationLedger } catch {}
    throw
  }
}
...
function Capture-V729Environment([string[]]$Names) {
  $Snapshot = @{}
...
function Restore-V729Environment($Snapshot) {
...
function Clear-V729DemoEnvironment {
...
Write-Host 'SAFE - sanitized gate ledger: artifacts\v7-29-certification-ledger.json' -ForegroundColor Yellow
...
Step 'Certification machine preflight' { node scripts/opsiqo85-v7-29-certification-preflight.mjs }
...
$script:CertificationLedger = Join-Path $Root 'artifacts\v7-29-certification-ledger.json'
Save-CertificationLedger
Step 'Locked dependency installation' { npm ci --no-audit --no-fund }
Step 'Locked toolchain post-install preflight' { node scripts/opsiqo85-v7-29-certification-preflight.mjs --post-install }
Step 'V7.29 Final Release Attestation audit' { npm run opsiqo85:opsiqo-one-v7.29:audit }
Step 'V7.29 translation inventory verification' { npm run opsiqo85:v7.29:translation-inventory:verify }
...
$PublicServer = $null
$PublicEnvSnapshot = Capture-V729Environment @('OPSIQO_A11Y_BASE_URL')
$PublicStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.29 - Built-app public browser accessibility smoke' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  Restore-V729Environment $PublicEnvSnapshot
...
$AuthEnvSnapshot = Capture-V729Environment $script:V729DemoEnvironmentNames
$AuthStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.29 - Authenticated emulator-backed accessibility UAT' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  npm run opsiqo85:v7.29:browser-a11y-auth
  ...
  Restore-V729Environment $AuthEnvSnapshot
}
...
Step 'Deployment readiness report' { npm run opsiqo85:v7.29:deployment-readiness -- $script:CertificationLedger }
Step 'Final release attestation report' { npm run opsiqo85:v7.29:final-attestation }
Save-CertificationLedger
node scripts/opsiqo85-v7-29-certification-summary.mjs $script:CertificationLedger
Write-Host ''
Write-Host ('=' * 76) -ForegroundColor Green
Write-Host ' OPSIQO ONE V7.29 AUTOMATED CERTIFICATION PASS' -ForegroundColor Green
Write-Host ('=' * 76) -ForegroundColor Green
...
Write-Host ' Evidence: artifacts\v7-29-final-release-attestation.json' -ForegroundColor Yellow
