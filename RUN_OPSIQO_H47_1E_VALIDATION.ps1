...
Write-Host 'OPSIQO H47.1E EXPO DOCTOR WORKSPACE ISOLATION VALIDATION' -ForegroundColor Magenta
...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ('opsiqo-h471e-mobile-' + [Guid]::NewGuid().ToString('N'))
$TempMobile = Join-Path $TempRoot 'mobile'
$PreviousCI = $env:CI

try {
  Write-Host 'Creating isolated Employee Mobile certification workspace...' -ForegroundColor Cyan
  New-Item -ItemType Directory -Path $TempMobile -Force | Out-Null

  Get-ChildItem -LiteralPath $MobileSource -Force |
    Where-Object { $_.Name -notin @('node_modules', '.expo', 'web-build', 'package-lock.json') } |
    Copy-Item -Destination $TempMobile -Recurse -Force

  Push-Location $TempMobile
  try {
    Write-Host 'Installing isolated H47.1E mobile dependencies and generating temporary lock evidence...' -ForegroundColor Cyan
    npm install --no-audit --no-fund
    if ($LASTEXITCODE -ne 0) { throw 'Mobile npm install failed.' }

    if (-not (Test-Path '.\package-lock.json' -PathType Leaf)) {
      throw 'Mobile dependency install did not produce package-lock.json.'
    }
...
Write-Host 'H47.1E OPSIQO EMPLOYEE MOBILE EXPO DOCTOR ISOLATION: PASS' -ForegroundColor Green
