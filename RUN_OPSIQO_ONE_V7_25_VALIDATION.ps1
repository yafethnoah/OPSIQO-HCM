...
$script:CertificationLedger = Join-Path ([System.IO.Path]::GetTempPath()) 'opsiqo-v7-25-certification-ledger.json'
...
function Step([string]$Name,[scriptblock]$Action) {
  ...
  Write-Host " OPSIQO ONE V7.25 - $Name" -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
...
function Clear-V725DemoEnvironment {
...
Set-Location $Root
Write-Host 'OPSIQO ONE v7.25 local certification' -ForegroundColor Cyan
...
Write-Host 'SAFE - sanitized gate ledger: artifacts\v7-25-certification-ledger.json' -ForegroundColor Yellow

Step 'Certification machine preflight' { node scripts/opsiqo85-v7-25-certification-preflight.mjs }
...
$script:CertificationLedger = Join-Path $Root 'artifacts\v7-25-certification-ledger.json'
Save-CertificationLedger
Step 'Locked dependency installation' { npm ci --no-audit --no-fund }
Step 'Locked toolchain post-install preflight' { node scripts/opsiqo85-v7-25-certification-preflight.mjs --post-install }
Step 'V7.25 Production Certification Closure audit' { npm run opsiqo85:opsiqo-one-v7.25:audit }
Step 'V7.25 translation inventory verification' { npm run opsiqo85:v7.25:translation-inventory:verify }
...
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.25 - Built-app public browser accessibility smoke' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  $PublicServer = Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31726' -PassThru -WindowStyle Hidden
  $Ready = $false
  foreach ($Attempt in 1..90) { try { $Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31726/signin' -UseBasicParsing -TimeoutSec 2; if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break} } catch { Start-Sleep -Seconds 1 } }
  if(-not $Ready){throw 'Built production server did not become reachable on port 31726.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31726'
...
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.25 - Authenticated emulator-backed accessibility UAT' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  $DemoServer=Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31727' -PassThru -WindowStyle Hidden
  $Ready=$false
  foreach($Attempt in 1..90){try{$Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31727/home' -UseBasicParsing -TimeoutSec 2;if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break}}catch{Start-Sleep -Seconds 1}}
  if(-not $Ready){throw 'Authenticated demo server did not become reachable on port 31727.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31727'
  npm run opsiqo85:v7.25:browser-a11y-auth
...
Write-Host ' OPSIQO ONE V7.25 CERTIFICATION PASS' -ForegroundColor Green
Write-Host ('=' * 76) -ForegroundColor Green
