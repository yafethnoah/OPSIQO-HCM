...
$script:CertificationLedger = Join-Path ([System.IO.Path]::GetTempPath()) 'opsiqo-v7-28-certification-ledger.json'
...
function Step([string]$Name,[scriptblock]$Action) {
  ...
  Write-Host " OPSIQO ONE V7.28 - $Name" -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  try {
    ...
    try { node scripts/opsiqo85-v7-28-certification-summary.mjs $script:CertificationLedger } catch {}
    throw
  }
}
...
function Capture-V728Environment([string[]]$Names) {
  $Snapshot = @{}
...
function Restore-V728Environment($Snapshot) {
...
function Clear-V728DemoEnvironment {
...
$script:V728DemoEnvironmentNames = @('NEXT_PUBLIC_OPSIQO_DEMO_MODE','NEXT_PUBLIC_OPSIQO_DEMO_ORG_ID','NEXT_PUBLIC_OPSIQO_ORG_ID','NEXT_PUBLIC_OPSIQO_USE_FIREBASE_EMULATORS','NEXT_PUBLIC_FIREBASE_PROJECT_ID','NEXT_PUBLIC_FIREBASE_AUTH_EMULATOR_URL','OPSIQO_DEMO_MODE','OPSIQO_DEMO_ORG_ID','OPSIQO_DEMO_UID','OPSIQO_DEMO_WORKER_ID','FIRESTORE_EMULATOR_HOST','FIREBASE_AUTH_EMULATOR_HOST','FIREBASE_STORAGE_EMULATOR_HOST','FIREBASE_PROJECT_ID','GOOGLE_CLOUD_PROJECT','OPSIQO_A11Y_BASE_URL')
Set-Location $Root
Write-Host 'OPSIQO ONE v7.28 local certification' -ForegroundColor Cyan
...
Write-Host 'SAFE - sanitized gate ledger: artifacts\v7-28-certification-ledger.json' -ForegroundColor Yellow
...
Step 'Certification machine preflight' { node scripts/opsiqo85-v7-28-certification-preflight.mjs }
...
$script:CertificationLedger = Join-Path $Root 'artifacts\v7-28-certification-ledger.json'
Save-CertificationLedger
Step 'Locked dependency installation' { npm ci --no-audit --no-fund }
Step 'Locked toolchain post-install preflight' { node scripts/opsiqo85-v7-28-certification-preflight.mjs --post-install }
Step 'V7.28 Deployment Readiness & Translation Closure audit' { npm run opsiqo85:opsiqo-one-v7.28:audit }
Step 'V7.28 translation inventory verification' { npm run opsiqo85:v7.28:translation-inventory:verify }
...
$PublicServer = $null
$PublicEnvSnapshot = Capture-V728Environment @('OPSIQO_A11Y_BASE_URL')
$PublicStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.28 - Built-app public browser accessibility smoke' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  $PublicServer = Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31732' -PassThru -WindowStyle Hidden
  $Ready = $false
  foreach ($Attempt in 1..90) { try { $Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31732/signin' -UseBasicParsing -TimeoutSec 2; if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break} } catch { Start-Sleep -Seconds 1 } }
  if(-not $Ready){throw 'Built production server did not become reachable on port 31732.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31732'
  ...
  Restore-V728Environment $PublicEnvSnapshot
...
$AuthEnvSnapshot = Capture-V728Environment $script:V728DemoEnvironmentNames
$AuthStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.28 - Authenticated emulator-backed accessibility UAT' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  $DemoServer=Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31733' -PassThru -WindowStyle Hidden
  $Ready=$false
  foreach($Attempt in 1..90){try{$Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31733/home' -UseBasicParsing -TimeoutSec 2;if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break}}catch{Start-Sleep -Seconds 1}}
  if(-not $Ready){throw 'Authenticated demo server did not become reachable on port 31733.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31733'
  npm run opsiqo85:v7.28:browser-a11y-auth
  ...
  Restore-V728Environment $AuthEnvSnapshot
}
...
Step 'Deployment readiness report' { npm run opsiqo85:v7.28:deployment-readiness -- $script:CertificationLedger }
Save-CertificationLedger
node scripts/opsiqo85-v7-28-certification-summary.mjs $script:CertificationLedger
Write-Host ''
Write-Host ('=' * 76) -ForegroundColor Green
Write-Host ' OPSIQO ONE V7.28 CERTIFICATION PASS' -ForegroundColor Green
Write-Host ('=' * 76) -ForegroundColor Green
