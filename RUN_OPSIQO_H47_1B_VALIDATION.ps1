...
Write-Host 'OPSIQO H47.1B HISTORICAL CERTIFICATION COMPATIBILITY VALIDATION' -ForegroundColor Magenta
...
try {
  if (-not (Test-Path '.\node_modules\.bin\tsc.cmd')) {
    Write-Host 'Installing isolated H47.1B mobile dependencies...' -ForegroundColor Cyan
    npm install --no-package-lock --no-audit --no-fund
    if ($LASTEXITCODE -ne 0) { throw 'Mobile npm install failed.' }
  }

  Write-Host 'Checking Expo dependency alignment without modifying source...' -ForegroundColor Cyan
...
Write-Host 'H47.1B OPSIQO EMPLOYEE MOBILE CERTIFICATION COMPATIBILITY: PASS' -ForegroundColor Green
