...
Write-Host " OPSIQO H48.5 LIVE ATTENDANCE SYNCHRONIZATION VALIDATION" -ForegroundColor Magenta
Write-Host "============================================================" -ForegroundColor Magenta

if ((git branch --show-current).Trim() -ne $Branch) {
    throw "STOP - expected H48.5 branch $Branch."
}

foreach ($Required in @(
    ".\package.json",
    ".\package-lock.json",
    ".\src\lib\time\attendance-sync.ts",
    ".\src\components\time-workspace.tsx",
    ".\src\components\frontline-operations-panel.tsx",
    ".\src\app\api\organizations\[orgId]\time\attendance-live\route.ts",
    ".\src\app\api\organizations\[orgId]\time\attendance-activity\route.ts",
    ".\scripts\opsiqo-h48-5-live-attendance-sync-audit.mjs",
    ".\tests\h48-5-live-attendance-sync.test.ts"
)) {
    if (-not (Test-Path -LiteralPath $Required -PathType Leaf)) {
        throw "STOP - required H48.5 file missing: $Required"
    }
}
...
Write-Host ""
Write-Host "=== H48.5 STATIC AUDIT ===" -ForegroundColor Cyan
npm run h48.5:attendance-sync:audit
if ($LASTEXITCODE -ne 0) { throw "STOP - H48.5 static audit failed." }

Write-Host ""
Write-Host "=== H48.5 TARGETED + TIME REGRESSION TESTS ===" -ForegroundColor Cyan
npm run test:h48.5
if ($LASTEXITCODE -ne 0) { throw "STOP - H48.5 targeted tests failed." }
...
Write-Host " H48.5 LIVE ATTENDANCE SYNCHRONIZATION VALIDATION: PASS" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
Write-Host "H48.5 audit: PASS" -ForegroundColor Green
Write-Host "H48.5 targeted/time tests: PASS" -ForegroundColor Green
Write-Host "H48.4 attendance visibility regression: PASS" -ForegroundColor Green
