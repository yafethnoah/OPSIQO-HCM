...
function Step([string]$Name,[scriptblock]$Action) {
  ...
  Write-Host " OPSIQO ONE V7.21 - $Name" -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  & $Action
...
function Clear-V721DemoEnvironment {
...
Set-Location $Root
Write-Host 'OPSIQO ONE v7.21 local certification' -ForegroundColor Cyan
...
Step 'V7.21 Document Policy Learning Knowledge audit' { npm run opsiqo85:opsiqo-one-v7.21:audit }
Step 'V7.21 translation inventory verification' { npm run opsiqo85:v7.21:translation-inventory:verify }
...
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.21 - Built-app public browser accessibility smoke' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
...
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.21 - Authenticated emulator-backed accessibility UAT' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  $DemoServer=Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31721' -PassThru -WindowStyle Hidden
  $Ready=$false
  foreach($Attempt in 1..90){try{$Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31721/home' -UseBasicParsing -TimeoutSec 2;if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break}}catch{Start-Sleep -Seconds 1}}
  if(-not $Ready){throw 'Authenticated demo server did not become reachable on port 31721.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31721'
  npm run opsiqo85:v7.21:browser-a11y-auth
...
Write-Host ' OPSIQO ONE V7.21 CERTIFICATION PASS' -ForegroundColor Green
Write-Host ('=' * 76) -ForegroundColor Green
