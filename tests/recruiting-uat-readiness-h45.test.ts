...
describe('H45 recruiting UAT readiness and auth reliability', () => {
  ...
  it('publishes an explicit H45 runtime marker', () => {
    const identity = read('src/lib/release/identity.ts');
    expect(identity).toContain("OPSIQO_FEATURE_RELEASE = process.env.OPSIQO_FEATURE_RELEASE || 'H45'");
    expect(identity).toContain("8.5-v7.32-H45");
  });
