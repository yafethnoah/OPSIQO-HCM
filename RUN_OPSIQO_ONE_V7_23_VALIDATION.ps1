...
function Step([string]$Name,[scriptblock]$Action) {
  ...
  Write-Host " OPSIQO ONE V7.23 - $Name" -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  & $Action
...
function Clear-V723DemoEnvironment {
...
Set-Location $Root
Write-Host 'OPSIQO ONE v7.23 local certification' -ForegroundColor Cyan
...
Step 'V7.23 Production Certification Hardening audit' { npm run opsiqo85:opsiqo-one-v7.23:audit }
Step 'V7.23 translation inventory verification' { npm run opsiqo85:v7.23:translation-inventory:verify }
...
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.23 - Built-app public browser accessibility smoke' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  $PublicServer = Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31724' -PassThru -WindowStyle Hidden
  $Ready = $false
  foreach ($Attempt in 1..90) { try { $Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31724/signin' -UseBasicParsing -TimeoutSec 2; if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break} } catch { Start-Sleep -Seconds 1 } }
  if(-not $Ready){throw 'Built production server did not become reachable on port 31724.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31724'
...
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.23 - Authenticated emulator-backed accessibility UAT' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  $DemoServer=Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31725' -PassThru -WindowStyle Hidden
  $Ready=$false
  foreach($Attempt in 1..90){try{$Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31725/home' -UseBasicParsing -TimeoutSec 2;if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break}}catch{Start-Sleep -Seconds 1}}
  if(-not $Ready){throw 'Authenticated demo server did not become reachable on port 31725.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31725'
  npm run opsiqo85:v7.23:browser-a11y-auth
...
Write-Host ' OPSIQO ONE V7.23 CERTIFICATION PASS' -ForegroundColor Green
Write-Host ('=' * 76) -ForegroundColor Green
