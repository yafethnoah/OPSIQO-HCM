...
$script:CertificationLedger = Join-Path ([System.IO.Path]::GetTempPath()) 'opsiqo-v7-30-certification-ledger.json'
...
function Step([string]$Name,[scriptblock]$Action) {
  ...
  Write-Host " OPSIQO ONE V7.30 - $Name" -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  try {
    ...
    try { node scripts/opsiqo85-v7-30-certification-summary.mjs $script:CertificationLedger } catch {}
    throw
  }
}
...
function Capture-V730Environment([string[]]$Names) {
  $Snapshot = @{}
...
function Restore-V730Environment($Snapshot) {
...
function Clear-V730DemoEnvironment {
...
$script:V730DemoEnvironmentNames = @('NEXT_PUBLIC_OPSIQO_DEMO_MODE','NEXT_PUBLIC_OPSIQO_DEMO_ORG_ID','NEXT_PUBLIC_OPSIQO_ORG_ID','NEXT_PUBLIC_OPSIQO_USE_FIREBASE_EMULATORS','NEXT_PUBLIC_FIREBASE_PROJECT_ID','NEXT_PUBLIC_FIREBASE_AUTH_EMULATOR_URL','OPSIQO_DEMO_MODE','OPSIQO_DEMO_ORG_ID','OPSIQO_DEMO_UID','OPSIQO_DEMO_WORKER_ID','FIRESTORE_EMULATOR_HOST','FIREBASE_AUTH_EMULATOR_HOST','FIREBASE_STORAGE_EMULATOR_HOST','FIREBASE_PROJECT_ID','GOOGLE_CLOUD_PROJECT','OPSIQO_A11Y_BASE_URL')
Set-Location $Root
Write-Host 'OPSIQO ONE v7.30 local certification' -ForegroundColor Cyan
...
Write-Host 'SAFE - sanitized gate ledger: artifacts\v7-30-certification-ledger.json' -ForegroundColor Yellow
...
Step 'Certification machine preflight' { node scripts/opsiqo85-v7-30-certification-preflight.mjs }
...
$script:CertificationLedger = Join-Path $Root 'artifacts\v7-30-certification-ledger.json'
Save-CertificationLedger
Step 'Locked dependency installation' { npm ci --no-audit --no-fund }
Step 'Locked toolchain post-install preflight' { node scripts/opsiqo85-v7-30-certification-preflight.mjs --post-install }
Step 'V7.30 Final Source Closure audit' { npm run opsiqo85:opsiqo-one-v7.30:audit }
Step 'V7.30 translation inventory verification' { npm run opsiqo85:v7.30:translation-inventory:verify }
...
$PublicServer = $null
$PublicEnvSnapshot = Capture-V730Environment @('OPSIQO_A11Y_BASE_URL')
$PublicStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.30 - Built-app public browser accessibility smoke' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  Restore-V730Environment $PublicEnvSnapshot
...
$AuthEnvSnapshot = Capture-V730Environment $script:V730DemoEnvironmentNames
$AuthStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.30 - Authenticated emulator-backed accessibility UAT' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  npm run opsiqo85:v7.30:browser-a11y-auth
  ...
  Restore-V730Environment $AuthEnvSnapshot
}
...
Step 'Human sign-off schema check' { npm run opsiqo85:v7.30:human-signoff:validate }
Step 'Deployment readiness report' { npm run opsiqo85:v7.30:deployment-readiness -- $script:CertificationLedger }
Step 'Final release attestation report' { npm run opsiqo85:v7.30:final-attestation }
Save-CertificationLedger
node scripts/opsiqo85-v7-30-certification-summary.mjs $script:CertificationLedger
Write-Host ''
Write-Host ('=' * 76) -ForegroundColor Green
Write-Host ' OPSIQO ONE V7.30 AUTOMATED CERTIFICATION PASS' -ForegroundColor Green
Write-Host ('=' * 76) -ForegroundColor Green
...
Write-Host ' Evidence: artifacts\v7-30-final-release-attestation.json' -ForegroundColor Yellow
