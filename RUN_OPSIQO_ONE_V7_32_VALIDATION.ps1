...
$script:CertificationLedger = Join-Path ([System.IO.Path]::GetTempPath()) 'opsiqo-v7-32-certification-ledger.json'
...
function Save-CertificationLedger {
  ...
  [System.IO.File]::WriteAllText($script:CertificationLedger, $Json, (New-Object System.Text.UTF8Encoding($false)))
}
...
function Step([string]$Name,[scriptblock]$Action) {
  ...
  Write-Host " OPSIQO ONE V7.32 - $Name" -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  try {
    ...
    try { node scripts/opsiqo85-v7-32-certification-summary.mjs $script:CertificationLedger } catch {}
    throw
  }
}
...
function Capture-V732Environment([string[]]$Names) {
  $Snapshot = @{}
...
function Restore-V732Environment($Snapshot) {
...
function Clear-V732DemoEnvironment {
...
$script:V732DemoEnvironmentNames = @('NEXT_PUBLIC_OPSIQO_DEMO_MODE','NEXT_PUBLIC_OPSIQO_DEMO_ORG_ID','NEXT_PUBLIC_OPSIQO_ORG_ID','NEXT_PUBLIC_OPSIQO_USE_FIREBASE_EMULATORS','NEXT_PUBLIC_FIREBASE_PROJECT_ID','NEXT_PUBLIC_FIREBASE_AUTH_EMULATOR_URL','OPSIQO_DEMO_MODE','OPSIQO_DEMO_ORG_ID','OPSIQO_DEMO_UID','OPSIQO_DEMO_WORKER_ID','FIRESTORE_EMULATOR_HOST','FIREBASE_AUTH_EMULATOR_HOST','FIREBASE_STORAGE_EMULATOR_HOST','FIREBASE_PROJECT_ID','GOOGLE_CLOUD_PROJECT','OPSIQO_A11Y_BASE_URL')
Set-Location $Root
Write-Host 'OPSIQO ONE v7.32 local certification' -ForegroundColor Cyan
...
Write-Host 'SAFE - sanitized gate ledger: artifacts\v7-32-certification-ledger.json' -ForegroundColor Yellow
...
Write-Host "SAFE - bootstrap certification ledger: $script:CertificationLedger" -ForegroundColor Yellow
Write-Host 'SAFE - root .env.local may remain as a certification-only local runtime overlay; its contents are never read, copied, hashed or printed by the clean-release gate.' -ForegroundColor Yellow

Step 'Windows PowerShell 5.1 compatibility audit' { node scripts/opsiqo85-v7-32-windows-powershell-compat-audit.mjs }
Step 'Certification machine preflight' { node scripts/opsiqo85-v7-32-certification-preflight.mjs }
Step 'Frozen source verification before install' { node scripts/source-manifest.mjs verify }
Step 'Clean release audit before install' { node scripts/opsiqo-clean-release-audit.mjs --certification-local-env-overlay }
$script:CertificationLedger = Join-Path $Root 'artifacts\v7-32-certification-ledger.json'
Save-CertificationLedger
Step 'Locked dependency installation' { npm ci --no-audit --no-fund }
Step 'Locked toolchain post-install preflight' { node scripts/opsiqo85-v7-32-certification-preflight.mjs --post-install }
Step 'V7.32 Final Runtime Closure audit' { npm run opsiqo85:opsiqo-one-v7.32:audit }
Step 'V7.32 Hotfix 11 hardening audit' { npm run opsiqo85:v7.32:hotfix11:audit }
Step 'V7.32 Hotfix 12 readiness summary secrecy audit' { npm run opsiqo85:v7.32:hotfix12:audit }
Step 'V7.32 Hotfix 13 bounded browser UAT audit' { npm run opsiqo85:v7.32:hotfix13:audit }
Step 'V7.32 Hotfix 14 isolated browser watchdog audit' { npm run opsiqo85:v7.32:hotfix14:audit }
Step 'V7.32 Hotfix 15 historical audit compatibility' { npm run opsiqo85:v7.32:hotfix15:audit }
Step 'V7.32 Hotfix 16 TypeScript watchdog bridge audit' { npm run opsiqo85:v7.32:hotfix16:audit }
Step 'V7.32 Hotfix 17 authenticated accessibility closure audit' { npm run opsiqo85:v7.32:hotfix17:audit }
Step 'V7.32 Hotfix 18 AI Copilot translation root audit' { npm run opsiqo85:v7.32:hotfix18:audit }
Step 'V7.32 Hotfix 19 shell locale and target-size audit' { npm run opsiqo85:v7.32:hotfix19:audit }
Step 'V7.32 translation inventory verification' { npm run opsiqo85:v7.32:translation-inventory:verify }
Step 'Semantic TypeScript' { npm run typecheck }
Step 'V7.32 targeted tests' { npm run test:opsiqo-one-v7.32 }
Step 'V7.32 Hotfix 11 targeted tests' { npm run test:opsiqo-one-v7.32-hotfix11 }
Step 'V7.32 Hotfix 12 targeted tests' { npm run test:opsiqo-one-v7.32-hotfix12 }
Step 'V7.32 Hotfix 13 targeted tests' { npm run test:opsiqo-one-v7.32-hotfix13 }
Step 'V7.32 Hotfix 14 targeted tests' { npm run test:opsiqo-one-v7.32-hotfix14 }
Step 'V7.32 Hotfix 15 targeted tests' { npm run test:opsiqo-one-v7.32-hotfix15 }
Step 'V7.32 Hotfix 16 targeted tests' { npm run test:opsiqo-one-v7.32-hotfix16 }
Step 'V7.32 Hotfix 17 targeted tests' { npm run test:opsiqo-one-v7.32-hotfix17 }
Step 'V7.32 Hotfix 18 targeted tests' { npm run test:opsiqo-one-v7.32-hotfix18 }
Step 'V7.32 Hotfix 19 targeted tests' { npm run test:opsiqo-one-v7.32-hotfix19 }
Step 'V7.31 targeted tests' { npm run test:opsiqo-one-v7.31 }
Step 'V7.30 targeted tests' { npm run test:opsiqo-one-v7.30 }
Step 'V7.29 targeted tests' { npm run test:opsiqo-one-v7.29 }
Step 'V7.28 targeted tests' { npm run test:opsiqo-one-v7.28 }
Step 'V7.27 targeted tests' { npm run test:opsiqo-one-v7.27 }
...
Step 'V7.31 regression' { npm run opsiqo85:opsiqo-one-v7.31:audit }
Step 'V7.30 regression' { npm run opsiqo85:opsiqo-one-v7.30:audit }
Step 'V7.29 regression' { npm run opsiqo85:opsiqo-one-v7.29:audit }
Step 'V7.28 regression' { npm run opsiqo85:opsiqo-one-v7.28:audit }
Step 'V7.27 regression' { npm run opsiqo85:opsiqo-one-v7.27:audit }
Step 'V7.26 regression' { npm run opsiqo85:opsiqo-one-v7.26:audit }
...
$PublicServer = $null
$PublicEnvSnapshot = Capture-V732Environment @('OPSIQO_A11Y_BASE_URL')
$PublicStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.32 - Built-app public browser accessibility smoke' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  $PublicServer = Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31742' -PassThru -WindowStyle Hidden
  $Ready = $false
  foreach ($Attempt in 1..90) { try { $Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31742/signin' -UseBasicParsing -TimeoutSec 2; if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break} } catch { Start-Sleep -Seconds 1 } }
  if(-not $Ready){throw 'Built production server did not become reachable on port 31742.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31742'
  ...
  Restore-V732Environment $PublicEnvSnapshot
...
$AuthEnvSnapshot = Capture-V732Environment $script:V732DemoEnvironmentNames
$AuthStarted = [System.Diagnostics.Stopwatch]::StartNew()
try {
  Write-Host ''
  Write-Host ('=' * 76) -ForegroundColor Magenta
  Write-Host ' OPSIQO ONE V7.32 - Authenticated emulator-backed accessibility UAT' -ForegroundColor Magenta
  Write-Host ('=' * 76) -ForegroundColor Magenta
  ...
  $DemoServer=Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm start -- -p 31743' -PassThru -WindowStyle Hidden
  $Ready=$false
  foreach($Attempt in 1..90){try{$Response=Invoke-WebRequest -Uri 'http://127.0.0.1:31743/home' -UseBasicParsing -TimeoutSec 2;if($Response.StatusCode -ge 200 -and $Response.StatusCode -lt 500){$Ready=$true;break}}catch{Start-Sleep -Seconds 1}}
  if(-not $Ready){throw 'Authenticated demo server did not become reachable on port 31743.'}
  $env:OPSIQO_A11Y_BASE_URL='http://127.0.0.1:31743'
  npm run opsiqo85:v7.32:browser-a11y-auth
  ...
  Restore-V732Environment $AuthEnvSnapshot
}
...
Step 'Human sign-off schema check' { npm run opsiqo85:v7.32:human-signoff:validate }
Step 'Deployment readiness report' { npm run opsiqo85:v7.32:deployment-readiness }
Step 'Final release attestation report' { npm run opsiqo85:v7.32:final-attestation }
Save-CertificationLedger
node scripts/opsiqo85-v7-32-certification-summary.mjs $script:CertificationLedger
Write-Host ''
Write-Host ('=' * 76) -ForegroundColor Green
Write-Host ' OPSIQO ONE V7.32 AUTOMATED CERTIFICATION PASS' -ForegroundColor Green
Write-Host ('=' * 76) -ForegroundColor Green
Write-Host ' Human accessibility, connector UAT, change approval and production deployment remain separately attested.' -ForegroundColor Yellow
Write-Host ' Evidence: artifacts\v7-32-final-release-attestation.json' -ForegroundColor Yellow
