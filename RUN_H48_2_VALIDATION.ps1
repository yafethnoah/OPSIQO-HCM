...
Write-Host " OPSIQO H48.2 TIME CLOCK PREFLIGHT VALIDATION" -ForegroundColor Magenta
Write-Host "============================================================" -ForegroundColor Magenta
...
foreach ($Required in @(
    ...
    ".\src\lib\time\clock-preflight.ts",
    ".\src\lib\time\offline-attendance.ts",
    ".\src\lib\time\service.ts",
    ".\scripts\opsiqo-h48-2-time-clock-preflight-audit.mjs",
    ".\tests\h48-2-time-clock-preflight.test.ts"
)) {
    if (-not (Test-Path -LiteralPath $Required -PathType Leaf)) {
        throw "STOP - required H48.2 file missing: $Required"
    }
}
...
Write-Host ""
Write-Host "=== H48.2 STATIC AUDIT ===" -ForegroundColor Cyan
npm run h48.2:time:audit
if ($LASTEXITCODE -ne 0) { throw "STOP - H48.2 static audit failed." }

Write-Host ""
Write-Host "=== H48.2 TARGETED + TIME REGRESSION TESTS ===" -ForegroundColor Cyan
npm run test:h48.2
if ($LASTEXITCODE -ne 0) { throw "STOP - H48.2 targeted tests failed." }

Write-Host ""
Write-Host "=== H48.1 REGRESSION TESTS ===" -ForegroundColor Cyan
...
Write-Host " H48.2 TIME CLOCK PREFLIGHT VALIDATION: PASS" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
Write-Host "H48.2 audit: PASS" -ForegroundColor Green
Write-Host "H48.2 targeted/time tests: PASS" -ForegroundColor Green
Write-Host "H48.1 regression: PASS" -ForegroundColor Green
